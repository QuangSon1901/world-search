...
const registerUser = createAsyncThunk('auth/registerUser', async (form) => {
    try {
        const res = await httpRequest.post('http://127.0.0.1:8000/api/register', form);
...
const loginUser = createAsyncThunk('auth/loginUser', async (form) => {
    try {
        const res = await httpRequest.post('http://127.0.0.1:8000/api/login', form);
...
const getUser = createAsyncThunk('auth/getUser', async () => {
    ...
    try {
        const res = await httpRequest.get(`http://127.0.0.1:8000/api/user-info`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
